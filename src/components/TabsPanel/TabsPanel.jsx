...
const TabsPanel = ({ tabsPanelVisible }) => {
	const [currentTabsState, setCurrentTabsState] = useState({
		...
		currentSpecialty: specialties.filter(
			specialty => specialty.direction_id === directions[0].id
		),
	})
	 ...
	 */
	const [showAllSpecializations, setShowAllSpecializations] = useState({
		status: false,
		state: undefined,
	})
	...
	const directionHover = id => {
		setCurrentTabsState({
			...
			currentSpecialty: specialties.filter(
				specialty => specialty.direction_id === id
			),
		})
	...
	const getCurrentSpecializations = specialtyId => {
		return specializations.filter(
			specialization =>
				specialization.direction_id === currentTabsState.currentDirection.id &&
				specialization.specialty_id === specialtyId
		)
	}
						...
						<div className={style.currentDirectionName}>
							{showAllSpecializations.status === true ? (
								<span className={style.styleBoxForSpecializationsView}>
									...
									/>
									<a href='#'>
										{currentTabsState.currentDirection.name} /{' '}
										{showAllSpecializations.state.name}
									</a>
								</span>
							) : (
								<span>
						...
						<div className={style.specialtySpecialization}>
							{showAllSpecializations.status === true ? (
								...
								<SpecialtySpecialization
									currentTabsState={currentTabsState}
									getCurrentSpecializations={getCurrentSpecializations}
